                    ...
                    <?php while ($row = mysqli_fetch_assoc($query2)): ?>
                        ...
                        <div id="modal-<?= $row['id_kegiatan'] ?>" class="modal fixed inset-0 bg-black bg-opacity-50 hidden flex items-center justify-center z-50" data-modal-id="<?= $row['id_kegiatan'] ?>">
                            <div class="bg-white rounded-lg p-6 w-full max-w-md relative">
                                ...
                                <!-- Komentar interakitf per kegiatan -->
                                <div class="mt-4">
                                    <h3 class="text-lg font-semibold">Komentar</h3>
                                    <form method="POST" action="komentar.php?id=<?= $row['id_kegiatan'] ?>" class="mt-2">
                                        <textarea name="komentar" rows="3" placeholder="Tulis komentar..." class="border p-2 rounded w-full"></textarea>
                                        <button type="submit" class="bg-blue-600 text-white px-3 py-2 rounded hover:bg-blue-700 mt-2">Kirim</button>
                                    </form>
                                </div>
                                <div class="mt-4">
                                    <h3 class="text-lg font-semibold">Daftar Komentar</h3>
                                    <ul class="mt-2">
                                            <li class="bg-gray-100 p-2 rounded mb-2">
                                                <strong>Contoh Komentar</strong>
                                                <p class="text-sm text-gray-600">Ini adalah komentar contoh.</p>
                                            </li>
                                    </ul>
                                </div>
                            </div>
